...
void priority(NODE** processesOrigin,int totalBurst){
  ...
  int index = 0;
  int clock = 0;
  priorityInsertReadyQueue(readyQueue,processes,clock,&index);
  printClock(clock);
  do{
    ...
    if(ptr != NULL){
      ...
      IO_request(readyQueue,waitingQueue,ptr);
        ...
        if(waitingQueue->out!=NULL)priority_IO_processing(readyQueue,waitingQueue);
...
void priorityInsertReadyQueue(QUEUE* readyQueue,NODE** processes,int clock,int* index){
  ...
  while(*index < PROCESSCOUNT){
    ...
    if(node -> process -> arrivalTime == clock){
      priorityInsertQueue(readyQueue,node);
      *index+=1;
    }else{
      break;
    }
...
void priority_IO_processing(QUEUE* readyQueue,QUEUE* waitingQueue){
  ...
  while(loc != NULL){
    ...
    if(loc -> process -> IO_burst == 0){
      ...
      loc->next = NULL;
      priorityInsertQueue(readyQueue,loc);
      loc = temp;
    }else{
      pre = loc;
      loc = loc -> next;
